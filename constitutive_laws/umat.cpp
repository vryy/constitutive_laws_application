                               ...
                               const Vector& ShapeFunctionsValues )
{
    ...
    case 0:
        ...
        if ( NPROPS[0] != 2 )
            KRATOS_ERROR << "LinearElastic umat material number material properties failure must be 2";
    ...
    case 1:
        ...
        if (NPROPS[0] < 4)
            KRATOS_ERROR << "Mises umat material number material properties failure must be >=4, E,nu,Syield,EPlasticYield";
    ...
    default:
        KRATOS_ERROR << "No umat material with id " << MaterialNumber[0] << " defined";
    }
}
...
{
    std::vector<double> DDSDDE(NTENS[0]*NTENS[0]);
    std::vector<double> DDSDDT(NTENS[0]);
    std::vector<double> DRPLDE(NTENS[0]);
    double TIM[2];
    double DTIME[1];
    int NPT[1];
    ...
    for ( int i = 0; i < NTENS[0]; i++ )
    {
        ...
        {
            DDSDDE[i*NTENS[0] + j] = 0.0;
        }
    }

    AlgorithmicTangent.clear();
    StressVector.clear();
    ...
    for ( int i = 0; i < NTENS[0]; i++ )
    {
        for ( int j = 0; j < NTENS[0]; j++ )
        {
            AlgorithmicTangent( i, j ) = DDSDDE[i + j*NTENS[0]]; // change from column-major to row-major
        }
