...
c       cons_lin.eq.true  -> numerical linearization via
c                            direct perturbation of dstran
c       cons_lin.eq.false -> continuum tangent stiffness
...
c
      logical prsw,elprsw,cons_lin,abaqus,chiara,check_ff,drcor
c
      integer i,error,maxnint,nfev,mario_DT_test
      integer nparms,nasvdim,nfasv,nydim,nzdim
      integer nasvy,nasvz,nyact,nzact
c
...
c	
      ...
      double precision deps_np1(6),depsv_np1
      double precision eps_n(6),epsv_n
      double precision norm_deps2,norm_deps,pp,qq,cos3t
      double precision zero,tol_f,fact_thres,p_thres,stran_lim,eps_debug
	double precision p_atm
c
      parameter (nasvdim  = 36)
      parameter (nydim    = 6+14)
      parameter (nzdim    = 14)
      parameter (tolintT  = 1.00d-6)
c
      ...
      parameter (cons_lin = .false.)
	parameter (abaqus = .false.)
c chiara
...
c
	...
	common /z_tolerance/tol_f
	common /z_check_yeld/check_ff
	common /z_drift_correction/drcor
	common /z_threshold_pressure/p_thres
c
	tol_f=1.0d-6
	check_ff=.false.
	drcor=.true.
...
c
      nparms=nprops
      call check_parms_DM(props,parms,nprops)
c
...
c
c	 if ((noel.eq.101).and.(npt.eq.1)) elprsw = .true.
c
      endif
c
...
c
      ...
      if (nyact.gt.nydim) then
        write(16,*) 'ERROR:nydim too small; program terminated'
        error=10
      elseif (nzact.gt.nzdim) then
        write(16,*) 'ERROR:nzdim too small; program terminated'
        error=10
      endif
c
...
c
      if (elprsw) then
        write(16,*) '================================================='
        write(16,*) '         Call of UMAT - DM SANISAND model:       '
        write(16,*) '================================================='
...
c			
	  if(.not.abaqus) then
	      write(6,*) 'analysis ended because number of time ' 
		  write(6,*) 'substeps exceeded maximum number allowed'
		  write(6,*) ' (maxnint)'
	      call xit
	  endif
	return
...
c        
        call tang_stiff(y,z,nyact,nasvy,nasvz,parms,nparms,DDtan)
c
      endif
c
	if(error.ne.0) then
...
c
      double precision dot_vect
c
	logical switch2,mario_DT_test
c
      external matmul
      double precision yf_DM
c 
      integer n,nasvz,nparms,i,n_drift,max_ndrift,error,switch
...
c
      ...
      double precision pp,p_thres_E,tenm3,p_thres
c
...
c-----------------------------------------------------------------------------
      implicit none
c
      integer error,n,nz,nasvy,nparms,i,nfev
c
	logical switch2,mario_DT_test
c
...
c-----------------------------------------------------------------------------
      ...
      external matmul
c 
      integer nparms,ny,nz,nasvy,i,j,error,switch2,iter,iter_max,switch4
...
c
	...
	double precision b0,d_sr,hh,db
      ...
      double precision mtrR,brack_mtrR,tol_f,tol_ff,tol_dil,Hvs
      double precision h_alpha(6),h_fab(6),HH_alpha(6,6),HH_fab(6,6)
	double precision yf_DM,ff0
c
...
c	  
	  if (dabs(p).gt.zero) then
	    b0=G0*h0*(one-c_h*void)/dsqrt(p/p_a)
	  else
	    b0=large
	  end if
c
...
c-----------------------------------------------------------------------------
	subroutine intersect_DM(y0,y1,y_star,n,parms,nparms,tol_ff,xi)
c-----------------------------------------------------------------------------
c
...
c
      implicit none
c
      integer n,nparms,maxiter,kiter,error,i,kiter_bis
c
	logical bisect
c
...
c a) Argyris function (Argyris=.true.) (as in the original paper)
c b) Van Eekelen function (Argyris=.false.) (more appropriate for high friction angles)
c
c       gth   = g(theta)
...
c-----------------------------------------------------------------------------
      implicit none
c 
      logical elprsw,plastic
c
      integer ntens,jj,kk
      integer n,nasvy,nasvz,nparms,nfev
      integer maxnint,error,mario_DT_test
c
...
c-----------------------------------------------------------------------------
      ...
      external matmul
c 
      integer nparms,ny,nz,i,error,iter,switch2,mario_DT_test
c
...
c
	...
	double precision b0,d_sr,hh,db,gth,dgdth
      double precision HHp,LDeR,Kp,Kpm1,norm2,norm
c
...
c		write(*,*)'subroutine plast_mod_DM: LDeR < 0'
	...
	if(Kp.lt.zero)then
		write(*,*)'function plast_mod: Kp < zero'
	error=3
	return
	endif
...
c-----------------------------------------------------------------------------
      implicit none
c
      logical elprsw,plastic,check_ff,drcor,mario,switch2,switch3,mario2
	logical mario_DT, mario_DT_test
c
      integer n,nasvy,nasvz,nparms,i,ksubst,kreject,nfev
      integer maxnint,error,attempt,maxnint_1
c
c
...
c
      ...
      double precision p_atm,tol_f,tol_ff,ff_tr,ff_k
      double precision T_k,DT_k,xi
      double precision kRK_1(n),kRK_2(n),kRK_3(n)
      double precision norm_R,S_hull
      double precision Fab(6),dev_fab(5),I1,f_p,absfp2,p_thres
...
c
c	
			write(*,*) 'ERROR: initial state is outside the YS'
			write(*,*) 'Subroutine RKF23_UPDATE'
c			write(*,*) 'f = ',ff_k_pp_k
			write(*,*) 'f = ',ff_k
			error = 10
			return 
...
c
c
		if ((norm_R.lt.err_tol).and.(attempt.ne.2).and.(attempt.ne.3)) then 				
c
...
c
c-----------------------------------------------------------------------------
      subroutine tang_stiff(y,z,n,nasvy,nasvz,parms,nparms,DD)
c-----------------------------------------------------------------------------
...
c-----------------------------------------------------------------------------
      implicit none
c 
      logical plastic,switch2,mario_DT_test
c
      integer n,nasvy,nasvz,nparms
      integer error
c
