...
!     3) cohesion, c
!     4) friction angle, phi [degrees]
!     5) dilation angle, psi [degrees]
!
...
!   CMNAME               -  name given on *material option. See above
...
!      4) Stress point on the apex (Not applicable for the Tresca Model)
!     Drucker-Prager:
!      1) Stress point on a single surface of the failure envelope
!      2) Stress point on the apex
!
!-----------------------------------------------------------------------
...
! Last modification: September 2009
!-----------------------------------------------------------------------
	implicit none
...
!-----------------------------------------------------------------------
     ...
     1 D(NTENS,NTENS), Dinv(NTENS,NTENS), Depc(NTENS,NTENS), K0,
     2 StrainElas(NTENS), DelStrainPlas(NTENS), PlastPar(3), 
     3 phi_d, psi_d, phi, psi, coh, E, nu, ElastPar(2),
	4 DSTRES(NTENS), SSE, SPE, DEE, TDE, DPE_eq, temp
...
!-----------------------------------------------------------------------
	...
	PlastPar(1) = (1.0_8+dsin(phi))/(1.0_8-dsin(phi))  ! Friction parameter
	PlastPar(2) = 2*coh*sqrt(PlastPar(1))    ! Uniaxial comp. str
	PlastPar(3) = (1.0_8+dsin(psi))/(1.0_8-dsin(psi))  ! Dilatation parameter
...
!-----------------------------------------------------------------------
	...
	!if (NSTATV.GE.2) STATEV(3) = DPE_eq			! Equivalent plastic strain increment
	!if (NSTATV.GE.3) STATEV(4) = STATEV(4) + DPE_eq ! Total equivalent plastic strain
...
!------------------------------------------------------------------------
	...
	integer(4), parameter :: DepFormLine = 1 ! Form of the consistent constitutive matrix on a line
					...
					! DepFormLine = 1: Modified "almost double singular" matrix on a line ("Tiln_b=axr" in the MatLab code)
...
!   ----- Principal stresses -----------------------------------------
      ...
      !				2/sqrt(3)
      !                                    2*pi/3
			SigP(1) = sqJ2 * dsin(lode + 2.094395102393195_8) + sigm
			SigP(2) = sqJ2 * dsin(lode)                       + sigm
			SigP(3) = sqJ2 * dsin(lode - 2.094395102393195_8) + sigm
		endif
...
!	-- Three distinct eigenvalues -----
     ...
     &			abs(SigP(1)-SigP(3)) > tol)			then
			do i = 1,2
				nl(1) = (Sigma(2)-SigP(i))
     &						* (Sigma(3)-SigP(i)) - Sigma(6)**2
				nl(2) = -Sigma(4) * (Sigma(3)-SigP(i))
     &						+ Sigma(6) * Sigma(5)
				nl(3) = Sigma(4) * Sigma(6)
     &						- (Sigma(2)-SigP(i)) * Sigma(5)
